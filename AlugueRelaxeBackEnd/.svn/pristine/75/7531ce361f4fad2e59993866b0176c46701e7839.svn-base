...
public class PainelPatrocinadorPlanoRecursos extends AbstractChain implements
		Chain<PublicidadeImovelDTO> {

	public void execute(PublicidadeImovelDTO context) {
		if(context.plano.recurso[PlanoRecursos.RECURSO_IMOVEL_PATROCINADOR].substring(0, 1).equals(Constantes.SIM)){
			// Obtem a quantidade do recurso
			int qtd = Integer.valueOf(context.plano.recurso[PlanoRecursos.RECURSO_IMOVEL_PATROCINADOR].substring(1, 4));
			String und = context.plano.recurso[PlanoRecursos.RECURSO_IMOVEL_PATROCINADOR].substring(4, 5);
			...
			if(und.equals("D")){
				...
				context.publicidade.tipoPublicidade = "PA";
