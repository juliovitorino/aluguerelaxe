...
import br.com.jcv.aluguerelaxe.client.formpesquisa.PublicidadeAVencerFormPesquisa;
...
public class LiquidarFaturaPublicidadeEditPanel extends AbstractFormEditPanel<LiquidarToolbar, WindowPanel, ImovelFichaCompletaVO> 
	...
	public LiquidarFaturaPublicidadeEditPanel(LiquidarToolbar toolbar) {
		super(toolbar);
		
		WindowPanel wpFiltro = new WindowPanel("Selecione a fatura de publicidade para registrar o pagamento",CLIENTE_FORM_TEMA);
		wpFiltro.setWidth("960px");
		PublicidadeAVencerFormPesquisa pavfp = new PublicidadeAVencerFormPesquisa();
		pavfp.addListener(this);
		wpFiltro.setComponenteCenter(pavfp);

		WindowPanel wpSituacaoFinanceira = new WindowPanel("Situa\u00e7\u00e3o Financeira da Publicidade",CLIENTE_FORM_TEMA,true,true,true);
		wpSituacaoFinanceira.setWidth("960px");
		wpSituacaoFinanceira.setComponenteCenter(new SituacaoFinanceiraAnuncioFormComposite());
		
		WindowPanel wpTrackerSF = new WindowPanel("Acompanhamento da Compra da Publicidade",CLIENTE_FORM_TEMA,true,true,true);
	...
	public void onSelecionarClick(RegDataGridVO registro){
		...
		AsyncCallback<ImovelFichaCompletaVO> callback = new AsyncCallback<ImovelFichaCompletaVO>() {
			...
			public void onSuccess(ImovelFichaCompletaVO result) {
				LiquidarFaturaPublicidadeEditPanel.this.callbackFicha = true;
				LiquidarFaturaPublicidadeEditPanel.this.ifcvo = result;
				update(result);
				update(LiquidarFaturaPublicidadeEditPanel.this.ifcvo, LiquidarFaturaPublicidadeEditPanel.this.ipfvo);
			}
		};
		...
		AsyncCallback<ImovelPlanoFaturaVO> callbackfatura = new AsyncCallback<ImovelPlanoFaturaVO>() {
			...
			public void onSuccess(ImovelPlanoFaturaVO result) {
				LiquidarFaturaPublicidadeEditPanel.this.callbackFatura = true;
				LiquidarFaturaPublicidadeEditPanel.this.ipfvo = result;
				update(result);
				update(LiquidarFaturaPublicidadeEditPanel.this.ifcvo, LiquidarFaturaPublicidadeEditPanel.this.ipfvo);
			}
		};
	...
	public void onLiquidarClick() {
		...
		cdm.addConfirmarDialogModalListener(new ConfirmarDialogModalListener() {
			...
			public void onSimClick() {
				LiquidarFaturaPublicidadeEditPanel.this.executeLiquidar();
			}
	...
	private void executeLiquidar() {
		...
		rpc.liquidarFatura(this.ipfvo.id, "P", callback);
	}
