...
import br.com.jcv.backend.workers.OfertaPublicidadePP;
 ...
 * <p>Busca a variavel configurada MAX_IMOVEL_SORTEIO_PUBL_PP</p>
 ...
 */
public class SorteioPublicidadePP extends Sorteio {

	private static Logger logger = Logger.getLogger(SorteioPublicidadePP.class); 
	private static final String TIPO_PUBLICIDADE = "PP";
	...
	public void executar() throws AlugueRelaxeException{
		...
		if (lst != null) {
			...
			try {
				...
				DateManagerBase d = DateManager.getDateManagerInstance(dataInicioPP.getDate());
				DateManagerBase dataFinalPP = DateManager.getDateManagerInstance(d.add(qtdDiasPrazoSorteio));
				...
				if (t.dataDiff(hoje.getDate(),dataInicioPP.getDate()) < 30){				
					...
					for (ImovelFichaCompletaDTO ifcdto : lst){
						...
						if (ifcdto.imagensImovelXG != null){
							...
							//---------------------------------------------------
							ImovelPlanoFaturaDTO ipfdto = ipfs.pesquisarUltimaFatura(ifcdto.imovel.id, "P");
							...
							//-----------------------------------------------------
							IOfertaPublicidade iop = new OfertaPublicidadePP();
							iop.execute(pub,"PP");
