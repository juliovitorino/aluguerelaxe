 ...
 */
public class PublicidadeDestaqueCache {

	private String dataControle = null;
	private List<ImovelPublicidadeVO> cachelst = null;
	private static PublicidadeDestaqueCache instance = null;
	private ServletContext contexto = null;
	
	private static final Logger logger = Logger.getLogger(PublicidadeDestaqueCache.class.getName());


	/* Construtor privado */
	private PublicidadeDestaqueCache(ServletContext event) {
		this.contexto = event;
		initCache();
	}
	
	public static PublicidadeDestaqueCache getInstance(ServletContext event) {
		if (instance == null) {
			synchronized (PublicidadeDestaqueCache.class) {
				if (instance == null) {
					instance = new PublicidadeDestaqueCache(event);
				}
			}
		}
		return instance;
	...
	public List<ImovelPublicidadeVO> getPublicidadeCache() {
		...
		if (! this.dataControle.equals(dataAtual)) {
			synchronized (this) {
				if (! this.dataControle.equals(dataAtual)) {
					logger.info("***********************************");
					logger.info("* Atualizando o cache de destaque *");
					logger.info("***********************************");
					initCache();
				}
			}
	...
	private void initCache() {
		try {
			...
			if ((publicidades != null) && (publicidades.size() > 0)) {
				...
				logger.info(publicidades.size() + " Imóvel(eis) para publicidade destaque");
				...
				logger.info("******************************************");
				logger.info("* Cache de Publicidade Destaque Iniciado *");
				logger.info("******************************************");
			} else {
				this.cachelst = null;
				logger.info("Não foram encontradas publicidades destaque.");
			}
