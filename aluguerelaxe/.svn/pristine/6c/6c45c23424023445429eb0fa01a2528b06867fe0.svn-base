...
public class PlanosTipoPublicidadeListBox extends ComponenteListBox 
							implements AsyncCallback<List<PlanoVO>>{

	public PlanosTipoPublicidadeListBox() {
		update();
	}
	...
	public void update(){
		...
		rpc.listarPlanosPorTipo("P", callback);
	}
	...
	public void onSuccess(List<PlanoVO> result) {
		if (result.size() > 0) { 
			...
			for (PlanoVO vo : result) {
				this.addItem(vo.nome, String.valueOf(vo.id));
			}
		}
	}
}
