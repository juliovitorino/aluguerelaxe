...
public class OfertaPublicidadePP extends Robot implements IOfertaPublicidade {
	
	public static final String ROBOT_NOME = "OFERTA_PUBLICIDADE_PP";

	public OfertaPublicidadePP() {
		super(ROBOT_NOME, "PROMOTOR DE PUBLICIDADE PP");
	}
	...
	public void execute(PublicidadeImovelDTO pidto, String tipo) throws AlugueRelaxeException {
		...
		try {
			...
			dmbdi.add(2);
			int diasOferta = Integer.valueOf(VariavelCache.getInstance().getValor(VariavelConstantes.DIAS_OFERTA));
			...
			ImovelPlanoFaturaDTO ipfdto = ipfs.pesquisarUltimaFatura(pidto.fichaImovel.imovel.id, "P");
	...
	public void notificar(PublicidadeImovelDTO pidto, ImovelPlanoFaturaDTO ipfdto) throws AlugueRelaxeException {
		...
		cps.put(TemplateConstantes.TAGC_DESCRICAO_DO_PLANO,pidto.plano.descricao);
		...
		try {
			...
			conteudo.put(TemplateConstantes.TAGC_TIPO_PUBLICIDADE,"PRIMEIRA PAGINA");
		...
		Email email = EmailFactory.getInstanceEmailOfertaPublicidadePP(conteudo);
		...
		email.enviar(lst, null, "[ALUGUERELAXE] - Oferta de Publicidade imovel #" + pidto.fichaImovel.imovel.id, null);
	}
