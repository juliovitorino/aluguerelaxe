...
public class SuperBannerPlanoRecursos extends AbstractChain implements Chain<PublicidadeImovelDTO> {
	
	public void execute(PublicidadeImovelDTO context) {
		if(context.plano.recurso[PlanoRecursos.RECURSO_SUPERBANNER].substring(0, 1).equals(Constantes.SIM)){
			// Obtem a quantidade do recurso
			int qtd = Integer.valueOf(context.plano.recurso[PlanoRecursos.RECURSO_SUPERBANNER].substring(1, 4));
			String und = context.plano.recurso[PlanoRecursos.RECURSO_SUPERBANNER].substring(4, 5);
			int dias = 30;
			...
			if(und.equals("D")){
				...
				context.publicidade.tipoPublicidade = "SB";
