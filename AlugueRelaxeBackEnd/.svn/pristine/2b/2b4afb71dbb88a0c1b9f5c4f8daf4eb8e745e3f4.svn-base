...
/**
 * Template para alerta 24h
 * @author Julio
 *
 */
public class TemplateEmailAlerta24h extends Template {

	public TemplateEmailAlerta24h(Map<String,String> conteudo) throws  AlugueRelaxeException {
		super(VariavelCache.getInstance().getValor(VariavelConstantes.TEMPLATE_PATH).concat("template-email-alerta24h.html"));
	...
	private void inicializaTags(Map<String, String> conteudo) {
		if (conteudo != null) {
			...
			if (conteudo.containsKey(TemplateConstantes.TAGC_TITULO_IMOVEL_ALERTA24H)) {
				this.adicionarParametro(TemplateConstantes.TAGC_TITULO_IMOVEL_ALERTA24H, conteudo.get(TemplateConstantes.TAGC_TITULO_IMOVEL_ALERTA24H));
			}
			...
			if (conteudo.containsKey(TemplateConstantes.TAGC_EMAIL_VISITANTE)) {
				this.adicionarParametro(TemplateConstantes.TAGC_EMAIL_VISITANTE, conteudo.get(TemplateConstantes.TAGC_EMAIL_VISITANTE));
			}
			if (conteudo.containsKey(TemplateConstantes.TAGC_TELEFONE_VISITANTE)) {
				this.adicionarParametro(TemplateConstantes.TAGC_TELEFONE_VISITANTE, conteudo.get(TemplateConstantes.TAGC_TELEFONE_VISITANTE));
			}
